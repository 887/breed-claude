...
Hard-won, one rule each. State the rule, not the incident.
...
and the lane resumes when it closes. A competent integrator re-checks the head
before merging, so a push arriving *before* it starts is free; only a mid-gate
push wastes a run.

Blanket freezes are worse than the collision they prevent: "one push per PR" and
"hold until it merges" both idled lanes for twenty-plus minutes per merge. Make
the exclusion window as small as the real conflict, and put the signalling duty
on whoever can see it.

**The queue never stops for a lane.** Mainline does not wait for a long-running
refactor; the branch that diverged carries the rebase burden. A lane that cannot
keep up freezes *itself*, lands, then resumes — and lands smaller batches so it
never gets deep enough to matter.

**Order ready PRs deepest-first.** A small PR re-rebases in minutes; a forty-commit
one costs its lane an hour. Merging cheap-first and invalidating the expensive one
is the worst available order and the default one.

**The integrator's queue is stale by construction** — it gates one PR for fifteen
to twenty minutes while four lanes push. Make re-listing a literal command it runs
first each cycle, not a habit. An integrator reporting "nothing to do" while PRs
are open is the tell.

## Branch hygiene

**A merged PR's branch is finished forever.** Pushing to it strands the commit
...
moved on. Nobody is wrong and nothing moves.

**A lane branch needs a drain, not abolishing.** Measure depth with
`rev-list --count trunk..lane` on a cadence; an empty PR list is not evidence.

**Never restore a shared file wholesale from an older base.** On the ledger, an
allow-list, or a generated corpus it silently reverts other lanes' work.
Reapply your own edits as targeted changes.

**Warn lanes before rebasing a shared branch**, and enumerate a lane's backlog
yourself — a lane recalls the branches it is thinking about, not the ones it
forgot.

## Verification

**Judge a long job by its artifact, not its process table.** A live compiler
proves the machine is busy, not that your command is doing what you meant. Read
the output file early.

**Prove a search or diff is live before recording an absence.** An empty
`git diff` can mean "identical" or "the invocation was wrong" — one lane got empty
diffs on four files differing by 1.4MB. A uniform sweep of null results indicts
the instrument.

**Santa's status checks must be read-only.** In jj, ordinary commands snapshot
the working copy — polling `jj git fetch` in the integrator's checkout mutates it.
Use plain `git` and the forge CLI.

**A raw count is not an impact measure.** 341 invisible call sites resolved to one
uncredited row; 42 offending rows were 22 genuine. Lead with measured impact.

**Say which tree a number came from.** "30 locally, lands on merge" and "main is
at 30" are indistinguishable as bare figures.

## Ledgers and ratchets

**The implementer ticks, on the branch, in the change that lands the work.** Not
after the merge — a correction afterwards is a ledger change arriving without the
code that justifies it. There is no "the ID does not exist yet": a durable change
ID is stable from creation.

**Only the phase reservation lands ahead of the work.** New substeps and their
ticks ride the branch and merge with the code they describe.

**A ratchet reaching zero is a claim — ask what the zero is made of.** Zero means
every site is fixed, genuinely exempt, or blocked by a real blocker.
"Outstanding but annotated" is not a fourth category. A *true* reason on the wrong
disposition is more dangerous than a false one: it reads as considered.

**A stale citation and a false tick look identical — check the tree.** Work
shipped and the cited commit rewritten is a re-cite; work never shipped is an
untick. Durable IDs survive rebase, not abandonment.

**A phase cannot be marked DONE in the PR that completes it** where the gate
checks evidence against trunk. Closing always takes a follow-up commit.

## Driving helpers

**A live goal outranks your messages.** To change what a helper does, replace the
goal — a message competes with it and loses. If a helper ignores you twice, check
what you told it to do *persistently*.

**Slash commands need bracketed paste.** The TUI's slash-picker eats the first
characters of `send-keys`, so `/goal …` arrives as a plain message and silently
does nothing. Use `load-buffer` + `paste-buffer -p`, and verify the echo.

**Autocompact can silently fail.** Frozen context *and* zero processes *and* no
output, while still accepting input, is a wedge — send `/compact`. Context still
moving, or live processes, is work; leave it. A sibling that crossed the same
range cleanly is the clincher.

**Say both halves of any hold.** "Do not push" reads as "stop working" unless you
name what continues.

**Separate landing cadence from task selection.** "Land smaller" invites a helper
to start asking which task to take. Cadence is Santa's; selection is the lane's.

**Feedback that sounds like "you overstepped" narrows autonomy in whatever
direction the helper guesses.** Name the axis you meant.

## Findings and escalation

**When a gate blocks the integrator, it reports — it does not rephrase.** But
establish a bypass actually happened before calling one: a refusal followed by a
differently-spelled success is equally consistent with fixing what the gate
objected to.

**The integrator defers too, and its deferrals look like diligence.** "Flagging
for whoever picks this up next" is the forbidden option, dressed as hygiene.
Remove the excuse — hand over the resolved evidence and the exact command.

**Retarget a stacked child when its parent merges.** That is what success looks
like partway through a stack, not a mis-basing error. A base wrong for any other
reason goes back to the lane.
